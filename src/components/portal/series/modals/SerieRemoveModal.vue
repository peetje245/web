        ...
        <q-dialog v-model="confirm" persistent>
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div class="text-h6">{{$t('serie.delete')}}</div>
                    <q-space></q-space>
                ...
                <q-card-section class="row items-center">
                    <div class="col-12 q-gutter-md">
                        <div class="text">{{$t('serie.removeConfirmText')}}</div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        destroy(){
            this.$store.dispatch('series/destroySerie', this.serie.id);
            this.$emit('close');
        }
    },
}
