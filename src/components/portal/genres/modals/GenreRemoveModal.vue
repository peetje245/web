        ...
        <q-dialog v-model="confirm" persistent>
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div class="text-h6">{{$t('genre.delete')}}</div>
                    <q-space></q-space>
                ...
                <q-card-section class="row items-center">
                    <div class="col-12 q-gutter-md">
                        <div class="text">{{$t('genre.removeConfirmText')}}</div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        destroy(){
            this.$store.dispatch('genres/destroyGenre', this.genre.id);
            this.$emit('close');
        }
    },
}
